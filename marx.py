...
#Coleção 'Marx & Engels', 2013, prefácios-capítulo 25

#Abre o arquivo .txt com texto referente ao intervalo de págs. indicado acima
with open ('marx.txt', 'r') as meuarquivo:
...
#Retirei as seguintes entradas do dicionário frequência, pois julguei que pouco contribuiam em termos de informação transmitida
del frequencia['”']
del frequencia['“']
del frequencia['–']
del frequencia['ser']
...
wc = WordCloud(
    background_color='white', width=1000, height=800, max_words=100, relative_scaling=0.5, colormap='plasma', normalize_plurals=False
    ).generate_from_frequencies(frequencia)

#Plota o gráfico c/ título, subtítulo e nota de rodapé
plt.imshow(wc)
plt.axis('off')
plt.suptitle('O Capital em 1000 palavras', horizontalalignment='center', weight='bold')
plt.title('Gráfico construído a partir da obra: O Capital (livro I) | Autor: Karl Marx', size='xx-small')
plt.figtext(0.5, 0.05, 'Fonte: vide código-fonte em bitly.com/wordcloud-marx\nElaboração: Paulo André Silveira Jr. (Twitter: @asjr_p) | Terraço Econômico (Twitter: @terracoecon)', size='x-small', horizontalalignment='center')
plt.show()
