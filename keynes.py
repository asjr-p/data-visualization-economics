...
#Coleção 'Os Economistas', 1996, págs. 27-349

#Abre o arquivo .txt com texto referente ao intervalo de págs. indicado acima
with open ('keynes.txt', 'r') as meuarquivo:
...
wc = WordCloud(
    background_color='white', width=1000, height=800, max_words=1000, relative_scaling=0.5, colormap='cividis', normalize_plurals=False
    ).generate_from_frequencies(frequencia)

#Plota o gráfico c/ título, subtítulo e nota de rodapé
plt.imshow(wc)
plt.axis('off')
plt.suptitle('A Teoria Geral em 1000 palavras', horizontalalignment='center', weight='bold')
plt.title(
    'Gráfico construído a partir da obra: A Teoria Geral do Emprego, do Juro e da Moeda | Autor: John M. Keynes', size='xx-small')
plt.figtext(0.5, 0.05, 'Fonte: vide código-fonte em bitly.com/wordcloud-keynes\nElaboração: Paulo André Silveira Jr. (Twitter: @asjr_p) | Terraço Econômico (Twitter: @terracoecon)', size='x-small', horizontalalignment='center')
plt.show()
